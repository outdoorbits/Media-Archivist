...
class mail(object):
	...
	def __init__(self,setup):
		...
		self.conf_MAIL_SECURITY 	= self.setup.get_val('conf_MAIL_SECURITY')
		self.conf_MAIL_TIMEOUT 	= self.setup.get_val('conf_MAIL_TIMEOUT')
	...
	def sendmail(self, Subject, TextPlain, TextHTML='', File_name=None, File_content_disposition='attachment', File_content_ID=''):
		...
		if not self.mail_configured():
			print('Mail not fully configured.')
			return
		...
		if self.conf_MAIL_SECURITY == 'SSL':
			...
			try:
				server = smtplib.SMTP_SSL(self.conf_SMTP_SERVER, self.conf_SMTP_PORT, context=context, timeout=self.conf_MAIL_TIMEOUT)
				server.login(self.conf_MAIL_USER, self.conf_MAIL_PASSWORD)
			except Exception as e:
				print(f"Mail error while defining SSL smtp server: {e}")

		else:
			# STARTTLS
			try:
				server = smtplib.SMTP(self.conf_SMTP_SERVER, self.conf_SMTP_PORT, timeout=self.conf_MAIL_TIMEOUT)
				server.ehlo()
				...
				print(f"Mail error while defining STARTTLS smtp server: {e}")
		...
		if self.conf_MAIL_HTML:
			...
			try:
				...
				print(f"Error sending HTML mail: {e}")
...
"""
			try:
				...
				print(f"Error sending plain text mail: {e}")

		try:
			server.quit()
		except Exception as e:
				print(f"Mail error while disconnecting from smtp server: {e}")
